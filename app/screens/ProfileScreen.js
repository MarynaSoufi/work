...
const validationSchema = Yup.object().shape({
  ...
  city: Yup.string().min(4).label('City'),
...
export default function ProfileScreen() {
  ...
  const handleSubmit = async (info) => {
    ...
    await firestore
      ...
      .update({
        displayName:
          info.userName !== '' ? info.userName : currentUser.displayName,
        email: info.email !== '' ? info.email : currentUser.email,
        firstName:
          info.firstName !== '' ? info.firstName : currentUser.firstName,
        lastName: info.lastName !== '' ? info.lastName : currentUser.lastName,
        age: info.age !== '' ? info.age : currentUser.age,
        city: info.city !== '' ? info.city : currentUser.city,
      });
    setLoading(false);
  };
        ...
        <ScrollView style={styles.container}>
          ...
          <AppForm
            initialValues={{
              ...
              password: currentUser.password ? currentUser.pasword : '',
          ...
          >
            ...
            <AppFormField
              ...
              maxLength={8}
              placeholder={
                currentUser.firstName ? currentUser.firstName : 'FirstName'
              }
              name="firstName"
            />
            <AppFormField
              ...
              maxLength={255}
              placeholder={
                currentUser.lastName ? currentUser.lastName : 'LastName'
              }
              name="lastName"
            />
            <AppFormField
