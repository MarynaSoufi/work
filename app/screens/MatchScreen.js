      ...
      data
        ?.map((i) => {
          ...
          if (usersData?.data) {
            ...
            if (foundUser) {
              setting.rating =
                foundUser.rating.reduce((a, b) => a + b, 0) /
                foundUser.rating.length;
            } else {
              setting.rating = 3;
            }
          }
        ...
        .filter((f) => {
          const result =
            ...
            (!f.myLocation ||
              !dataItem.myLocation ||
              getDistance(
                dataItem.myLocation.latitude,
                dataItem.myLocation.longitude,
                f.myLocation.latitude,
                f.myLocation.longitude
              ) <= dataItem.range);
          return result;
        })
    );
          ...
          {match?.length ? (
            ...
            <View style={styles.noMatches}>
              <Text style={styles.noMatchesText}>
                You don't have any matches, try to change your settings!
              </Text>
            </View>
          )}
        </View>
