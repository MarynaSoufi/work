...
import { Screen, Button, ListMessage, ListItemSeparator, ActivityIndicator } from '../components';
    ...
    if(chat.id) {
      if(text) {
        ...
        if(internetConnectionIsActive){
          firestore.collection('chatMessages').doc(messagesId).collection('messages').add(messageRequest)
        } else {
    ...
    try {
      ...
      storage.ref(path).put(file).then(
        async (snapshot) => {
          ...
          await firestore.collection('chatMessages').doc(messagesId).collection('messages').add({
              id:id,
              imageUri: avatarUrl,
              timestamp: firebase.firestore.FieldValue.serverTimestamp(),
              sendFromId: currentUser?.id,
  ...
  <>
    ...
    <View style={styles.container}>
        ...
        <View style={styles.inputConteiner}>
          <TextInput style={styles.input} onChangeText={onChangeText} value={text} placeholder='Your message...'/>
          <Button styleBtn={styles.btn} iconName="paperclip" iconSize={24} onPress={() => setVisibleSheet(true)} iconColor={color.green}/>
          <Button styleBtn={styles.btn} iconName="send" iconSize={24} onPress={handlePress} iconColor={color.green}/>
      </View>
      </View>
      ...
      <Modal
          ...
          onRequestClose={() => {
            ...
            <View style={styles.bottomNavigationView}>
              <Pressable>
                <Button  onPress={selectImage} styleBtn={styles.camera} iconName="camera-image" iconSize={24}  iconColor={color.green}/>
              </Pressable>
              <Pressable>
                <Button onPress={showDialogLocation} styleBtn={styles.camera} iconName="map-marker" iconSize={24} iconColor={color.green}/>
              </Pressable>

              <Pressable>
                <Button onPress={openCamera} styleBtn={styles.camera} iconName="camera-plus" iconSize={24} iconColor={color.green}/>
              </Pressable>
