...
export default function ChatScreen({ route, navigation }) {
  ...
  const chatInfo = useFirestoreQuery(firestore.collection('chatMessages'));
  ...
  const setData = async () => {
    ...
    if (users) {
      ...
      if (userFound) {
        ...
        const chatsToSet = users
          .filter((f) => userFound.myChats?.includes(f.user))
          .map((u) => {
            if (chatInfo?.data) {
              const messagesId = [user.uid, u?.user].sort().join('_');
              const foundChatInfo = chatInfo.data.find(
                (i) => i.id === messagesId
              );
              if (foundChatInfo) {
                u.unread = foundChatInfo[`${user.uid}_unread`];
              }
            }
            return u;
          });
